...
function drawCombatFace(
  ...
  if (expression === 'not-yet-dead') {
    ...
    ctx.beginPath();
    ctx.moveTo(29.5 * scale, -36 * scale);
    ctx.lineTo(34 * scale, -41 * scale);
    ctx.lineTo(33 * scale, -34 * scale);
    ctx.lineTo(38 * scale, -37 * scale);
    ctx.stroke();
    ctx.strokeStyle = '#d28a78';
    ctx.lineWidth = Math.max(0.7, 0.75 * scale);
    ctx.globalAlpha = 0.78;
    ctx.beginPath();
    ctx.moveTo(30 * scale, -36 * scale);
    ctx.lineTo(34 * scale, -40.5 * scale);
    ctx.lineTo(33.4 * scale, -34.8 * scale);
    ctx.lineTo(37.4 * scale, -37.2 * scale);
    ctx.stroke();
    ctx.globalAlpha = 1;
    ctx.strokeStyle = '#111719';
