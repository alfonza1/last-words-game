...
export function TypeBar({ s, opts, input }: { s: GameState; opts: MatchOptions; input: string }) {
  ...
  const activeMatched =
    active && typed.length > 0 && isPrefix(input, active, opts) ? Math.min(typed.length, active.length) : 0;
          ...
          return (
            <span
              ...
              className={`rounded-md border bg-black/60 px-2.5 py-1 font-mono text-sm ${
                isFirst ? 'border-neon-green shadow-neon text-white/90' : 'border-white/10 text-white/45'
              }`}
            >
              {matched > 0 ? (
