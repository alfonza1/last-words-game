...
class ProfileStoreLeaderboardTest {
  ...
  @Test
  void leaderboardKeepsBestScoreAndHighestWpmForEachPlayer() {
    ...
    assertEquals(2_000, saved.getValue().score);
    assertEquals(5, saved.getValue().wave);
  }

  @Test
  void topTypingLeaderboardIsReadByScore() {
    when(leaderboard.findByRiddleOrderByScoreDesc(eq(false), any(Pageable.class)))
        .thenReturn(List.of(row(2L, false, 2_000, 5, 80), row(1L, false, 900, 2, 110)));

    List<LeaderboardEntry> entries = store.topLeaderboard(false, 20);

    assertEquals(2_000, entries.get(0).score());
    assertEquals(80, entries.get(0).wpm());
    assertEquals(900, entries.get(1).score());
    assertEquals(110, entries.get(1).wpm());
    verify(leaderboard).findByRiddleOrderByScoreDesc(eq(false), any(Pageable.class));
  }
